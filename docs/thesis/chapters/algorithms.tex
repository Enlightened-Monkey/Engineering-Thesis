...
\subsection{Izolacja $\alpha \beta J^{\beta}_{\phi_s}(s')$}
...
\ENDFOR
\ENDFOR
\STATE \textbf{Wynik:} $Q_n$ (QH) oraz $W_n$ (eksponencjalne); $\mu^\star(s) = \arg\max_a Q_n(s, a)$, $\phi_s^\star(s) = \arg\max_a W_n(s, a)$
\end{algorithmic}
\end{algorithm}
...
\item \textbf{Aktualizacja sterowania (Alg.~\ref{alg:qh-qlearning}):} Traktuj $W$ jako docelowe wartości eksponencjalne i wykonaj krok QH Q-learningu, aktualizując $Q$ oraz politykę pierwszego kroku $\mu$.
