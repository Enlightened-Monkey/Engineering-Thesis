...
Ten rozdział został opracowany na podstawie następujących prac i monografii \cite{eshwar2025,borkar2008,puterman2014}.
...
Kluczowym elementem Algorytmu 1 jest specyficzna konstrukcja celu temporalnego (ang. \textit{TD target}) oraz wykorzystanie dwóch skal czasowych do jednoczesnej aproksymacji pomocniczej funkcji wartości $W$ (dla dyskontowania wykładniczego) i docelowej funkcji wartości $J$ (dla dyskontowania quasi-hiperbolicznego).
...
\begin{itemize}
    ...
    \item $W_n(s')$ to aproksymata wartości wykładniczej w następnym stanie (tzw. \textit{bootstrap}).
\end{itemize}
...
Dzięki temu aproksymatory pozostają nieobciążone mimo generowania danych z innej polityki.
...
\paragraph{Szybka skala (Aproksymacja $W$):} Funkcja $W_n$ jest aktualizowana z krokiem $\eta_n$. Dąży ona do wartości $J_{\phi_s}^{\alpha, \beta}$ znacznie szybciej niż $J_n$, traktując $J_n$ jako quasi-statyczne.
...
Warunek $\frac{\theta_n}{\eta_n} \rightarrow 0$ zapewnia, że błędy aproksymacji $W_n$ ,,wygasają'' szybciej, dostarczając stabilnego celu dla wolniejszej aktualizacji $J_n$.
