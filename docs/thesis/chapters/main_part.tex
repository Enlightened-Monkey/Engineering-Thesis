...
Dla takiego dyskontowania, jest dobrze udowodnione\cite{puterman2014}, że istnieje stacjonarna polityka $\phi_s \in \Phi_s$, która spełnia
\[
...
W tej sekcji zdefiniujemy czym jest optymalna polityka w kontekście dyskontowania quasi-hiperbolicznego oraz przedstawimy bezmodelowy algorytm uczenia się optymalnej polityki.
\subsection{Definicja optymalnej polityki}
Optymalna polityka w dyskontowaniu QH to para polityk $(\mu^\star, \phi_s^\star)$ maksymalizująca wartość funkcji wartości $J^{\alpha, \beta}_{\mu, \phi_s}(s)$ dla każdego stanu $s \in S$. Formalnie:
\begin{equation}
(\mu^\star, \phi_s^\star) = \arg\max_{\mu \in \Phi_m} \max_{\phi_s \in \Phi_s} J^{\alpha, \beta}_{\mu, \phi_s}(s), \quad \forall s \in S.
\end{equation}

Żeby uzyskać optymalną politykę, wystarczy rozważać polityki postaci $(\mu, \phi_s)$ zgodnie z Twierdzeniem~\ref{thm:policy-reduction}.
...
Jako iż $\max\limits_{\phi_s} J^{\beta}_{\phi_s}(s)$ odpowiada optymalnej polityce dla dyskontowania wykładniczego, zapisujemy optymalną politykę jako $\phi^\star_\beta$. Konsekwentnie, optymalna polityka dla dyskontowania QH jest dana przez 
\[
...
\end{equation}
Zatem, jest wystarczające rozważać jedynie polityki deterministyczne przy poszukiwaniu optymalnej pary.
\end{theorem}
\begin{proof}
Najpierw musimy pokazać, że $\phi_s^\star$ może być wybrana jako deterministyczna. Dla dyskontowania wykładniczego jest dobrze znane, że istnieje optymalna polityka deterministyczna $\phi_\beta^\star$. Zatem skoro ustaliliśmy, że $\phi_s^\star = \phi_\beta^\star$, to $\phi_s^\star$ jest deterministyczna. \\
...
Drugi algorytm pozwala na znalezienie optymalnej polityki bez znajomości modelu środowiska. Tzn. znajdujemy optymalną parę polityk $(\mu^\star, \phi_s^\star)$ maksymalizującą wartość funkcji wartości $J^{\alpha, \beta}_{\mu, \phi_s}(s)$ dla każdego stanu $s \in S$ bazując jedynie na próbkach doświadczeń z interakcji ze środowiskiem, nie znając przejść $q$ ani funkcji nagród $r$.\\
